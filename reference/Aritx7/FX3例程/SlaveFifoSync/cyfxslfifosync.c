        ...
        void)
{
    ...
    apiRetStatus = CyU3PSetEpConfig(DATA_ENDPOINT_PRODUCER, &epCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    apiRetStatus = CyU3PSetEpConfig(DATA_ENDPOINT_CONSUMER, &epCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PSetEpConfig failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler (apiRetStatus);
    ...
    dmaCfg.size  = 16 * 1024;
    dmaCfg.count = 2;
	dmaCfg.prodSckId = PRODUCER_DATA_SOCKET;
    dmaCfg.consSckId = DATA_TX_PPORT_SOCKET;
    ...
    apiRetStatus = CyU3PDmaChannelCreate (&data_cons_to_prod_chan_handle,
    ...
    dmaCfg.size  = 16 * 1024; //increase buffer size for higher performance
    dmaCfg.count = 8; // increase buffer count for higher performance
    dmaCfg.prodSckId = DATA_RX_PPORT_SOCKET;
    dmaCfg.consSckId = CONSUMER_DATA_SOCKET;
    dmaCfg.cb = NULL;
    apiRetStatus = CyU3PDmaChannelCreate (&data_prod_to_cons_chan_handle,
               CY_U3P_DMA_TYPE_AUTO, &dmaCfg);
    ...
    apiRetStatus = CyU3PDmaChannelSetXfer (&data_cons_to_prod_chan_handle, CY_FX_SLFIFO_DMA_TX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer Failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
    }
    apiRetStatus = CyU3PDmaChannelSetXfer (&data_prod_to_cons_chan_handle, CY_FX_SLFIFO_DMA_RX_SIZE);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer Failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
	...
	CyU3PSetEpConfig(CTRL_ENDPOINT_PRODUCER, &epCfg);
	CyU3PSetEpConfig(CTRL_ENDPOINT_CONSUMER, &epCfg);

	/* Create a DMA AUTO channel for U2P transfer.
	 * DMA size is set based on the USB speed. */
	dmaCfg.size = size;
	dmaCfg.count = 2;
	dmaCfg.prodSckId = PRODUCER_CTRL_SOCKET;
	dmaCfg.consSckId = CTRL_COMM_PPORT_SOCKET;
	dmaCfg.dmaMode = CY_U3P_DMA_MODE_BYTE;
	dmaCfg.notification = 0;
	dmaCfg.cb = NULL;
	dmaCfg.prodHeader = 0;
	...
	/* Create a DMA AUTO channel for P2U transfer. */
	dmaCfg.prodSckId = CTRL_RESP_PPORT_SOCKET;
	dmaCfg.consSckId = CONSUMER_CTRL_SOCKET;
	dmaCfg.cb = NULL;
	...
	/* Set DMA channel transfer size. */
	CyU3PDmaChannelSetXfer(&ctrl_cons_to_prod_chan_handle, CY_FX_SLFIFO_DMA_TX_SIZE);
	CyU3PDmaChannelSetXfer(&ctrl_prod_to_cons_chan_handle, CY_FX_SLFIFO_DMA_RX_SIZE);
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplnActive)
            {
                if (wIndex == CY_FX_EP_PRODUCER)
                {
                    ...
                    CyU3PDmaChannelSetXfer (&data_cons_to_prod_chan_handle, CY_FX_SLFIFO_DMA_TX_SIZE);
                ...
                if (wIndex == CY_FX_EP_CONSUMER)
                {
                    ...
                    CyU3PDmaChannelSetXfer (&data_prod_to_cons_chan_handle, CY_FX_SLFIFO_DMA_RX_SIZE);
